...
import com.example.msl_b.config.auth.PrincipalDetails;
...
public class PostController {
    ...
    public Response getBoards() {
        return new Response("성공", "전체 게시물 리턴", postService.getBoards());
    }
    ...
    public Response getBoard(@PathVariable("id") BigInteger id) {
        return new Response("성공", "개별 게시물 리턴", postService.getBoard(id));
    }
    ...
    public Response write(@RequestBody PostDto boardDto, Authentication authentication) {
        PrincipalDetails principalDetails = (PrincipalDetails) authentication.getPrincipal();
        User user = principalDetails.getUser();
        return new Response("성공", "글 작성 성공", postService.write(boardDto, user));
    }
    ...
    public Response edit(@RequestBody PostDto boardDto, @PathVariable("id") BigInteger id, Authentication authentication) {
        ...
        PrincipalDetails principalDetails = (PrincipalDetails) authentication.getPrincipal();
        User user = principalDetails.getUser();
        if (user.getName().equals(postService.getBoard(id).getWriter())) {
            // 로그인된 유저의 글이 맞다면
            return new Response("성공", "글 수정 성공", postService.update(id, boardDto));
        } else {
    ...
    public Response delete(@PathVariable("id") BigInteger id, Authentication authentication) {
